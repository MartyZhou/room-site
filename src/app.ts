...
(() => {
  ...
  let prose: ProseContent = {};
  const proseCache: Record<string, ProseContent> = {};
  ...
  const photoBase = (i: number) => `public/images/photo-${String(i).padStart(2, '0')}`;
  const photoSrc = (i: number) => `${photoBase(i)}.jpeg`;
  const photoSrcWebp = (i: number) => `${photoBase(i)}.webp`;
  const photoPicture = (i: number, { alt = '', loading = 'lazy', cls = '' }: { alt?: string; loading?: string; cls?: string } = {}) => `
  ...
  //  - over HTTP (local server or a real host): fetched live, so edits to
  //    content/* show on refresh.
  //  - opened directly from disk (file://): browsers block fetch() of local
  //    files, so we read from the embedded bundle (content/bundle.js ->
  //    window.__CONTENT__), regenerated by `npm run bundle:content`.
  const isFile = location.protocol === 'file:';
  const fromBundle = (url: string): unknown => (window.__CONTENT__ || {})[url];

  const loadJSON = async <T>(url: string): Promise<T> => {
    if (isFile) {
      const b = fromBundle(url);
      if (b !== undefined) return b as T;
      throw new Error(`Not in bundle: ${url} — run npm run bundle:content`);
    }
    try {
  ...
  const loadText = async (url: string): Promise<string> => {
    if (isFile) {
      const b = fromBundle(url);
      if (b !== undefined) return b as string;
      throw new Error(`Not in bundle: ${url} — run npm run bundle:content`);
    }
    try {
      const res = await fetch(url, { cache: 'no-cache' });
  ...
  const loadProse = async (lang: Lang): Promise<ProseContent> => {
    if (proseCache[lang]) return proseCache[lang];
    const entries = await Promise.all(PROSE_KEYS.map(async (key): Promise<[string, string]> => {
      try {
  ...
  const escapeHTML = (s: unknown): string => String(s).replace(/[&<>"']/g, (c) => HTML_ESCAPES[c] ?? c);
  const mdInline = (s: string): string => s
    ...
    .replace(/(^|[^_])_([^_\n]+)_(?!_)/g, '$1<em>$2</em>')
    .replace(/`([^`\n]+)`/g, '<code>$1</code>');
  const renderMarkdown = (input: string | string[] | null | undefined): string => {
  ...
  const applyContent = (): void => {
    ...
    document.querySelectorAll<HTMLElement>('[data-bind]').forEach((node) => {
      const v = resolve(node.getAttribute('data-bind') || '');
      if (typeof v === 'string') node.textContent = v;
    });
    // data-bind-md: renders content/prose/{lang}/{slug}.md as HTML.
    document.querySelectorAll<HTMLElement>('[data-bind-md]').forEach((node) => {
      const slug = node.getAttribute('data-bind-md');
      const md = slug ? prose[slug] : undefined;
      if (md != null) node.innerHTML = renderMarkdown(md);
    });

    // Quick facts
    const factsEl = el('quick-facts');
    factsEl.innerHTML = '';
    (['guests', 'bedrooms', 'beds', 'baths'] as const).forEach((k) => {
    ...
    // Amenities
    const amenitiesEl = el('amenities-list');
    ...
    el('lb-close').setAttribute('aria-label', dict.lightbox?.close || 'Close');
    el('lb-prev').setAttribute('aria-label', dict.lightbox?.prev || 'Previous');
    el('lb-next').setAttribute('aria-label', dict.lightbox?.next || 'Next');
  ...
  const renderReviews = (): void => {
    ...
    if (!summaryEl || !gridEl || !reviews || !dict) return;
    const s = reviews.summary || {};
    const cats = s.categories || {};
    const basedOn = (dict.reviews?.basedOn || 'based on {n} reviews').replace('{n}', String(s.count || 0));
    ...
    if (tagsEl) {
      ...
      if (tags.length) {
        ...
        const tagsHtml = tags.map((t) => {
          const c = countTpl.replace('{n}', String(t.count));
  ...
  const buildGallery = (): void => {
    const galleryEl = el('gallery');
    galleryEl.innerHTML = '';
    galleryPhotos().forEach((n) => {
      const div = document.createElement('div');
      div.className = 'photo';
      const alt = (dict!.gallery?.photoOf || 'Photo {n}').replace('{n}', String(n));
  ...
  const overlay = {
    ...
    init() {
      this.el = el('photo-overlay');
      this.grid = el('po-grid');
      this.tabs = el('po-tabs');
      el('po-close').addEventListener('click', () => this.close());
      el('show-all').addEventListener('click', () => this.open());
    ...
    photosFor(category: string): number[] {
    ...
    renderGrid() {
      ...
      photos.forEach((i) => {
        ...
        const alt = altTpl.replace('{n}', String(i));
  ...
  const lb = {
    ...
    init() {
      this.el = el('lightbox');
      this.img = el<HTMLImageElement>('lb-img');
      this.source = el<HTMLSourceElement>('lb-source');
      this.count = el('lb-count');
    ...
    open(n: number) { this.current = n; this.render(); this.el!.hidden = false; document.body.style.overflow = 'hidden'; },
    ...
    render() {
      ...
      this.img!.alt = (dict!.gallery?.photoOf || 'Photo {n}').replace('{n}', String(this.current));
      this.count!.textContent = `${this.current} / ${site!.photos!.total}`;
    },
  };
  const openLightbox = (n: number) => lb.open(n);
  ...
  const wireLinks = (): void => {
    ...
    el<HTMLAnchorElement>('airbnb-link').href = site!.listings?.airbnb || '#';
    el<HTMLAnchorElement>('booking-link').href = site!.listings?.booking || '#';

    // Google Business (shown only if a URL is configured)
    const googleBtn = document.getElementById('google-link') as HTMLAnchorElement | null;
    if (googleBtn) {
    ...
    el<HTMLAnchorElement>('call-link').href = tel ? `tel:${tel}` : '#';
    el<HTMLAnchorElement>('sms-link').href = tel ? `sms:${tel}` : '#';

    // Reviews → Airbnb link
    const revLink = document.getElementById('reviews-airbnb-link') as HTMLAnchorElement | null;
    if (revLink) revLink.href = site!.listings?.airbnb || '#';

    // WhatsApp (shown only if site.contact.whatsapp === true)
    const waBtn = document.getElementById('whatsapp-link') as HTMLAnchorElement | null;
    if (waBtn) {
    ...
    const mapLink = document.getElementById('map-link') as HTMLAnchorElement | null;
    if (mapLink) {
    ...
    if (ld) { try { ldData = JSON.parse(ld.textContent || '{}'); } catch { /* ignore malformed JSON-LD */ } }
  ...
  const setLang = async (lang: string): Promise<void> => {
    const nextLang: Lang = lang === 'fr' || lang === 'en' ? lang : 'fr';
    currentLang = nextLang;
    [dict, prose] = await Promise.all([
      loadJSON<Dict>(`content/${nextLang}.json`),
      loadProse(nextLang),
    ]);
    applyContent();
    buildGallery();
    wireLinks();
    try { localStorage.setItem('lang', nextLang); } catch { /* storage unavailable */ }
  };

  // ---------- Theme switch ----------------------------------------------
  const THEMES = ['serenity', 'classique', 'azur', 'nuit'] as const;
  type Theme = typeof THEMES[number];
  const setTheme = (theme: string): void => {
    const nextTheme: Theme = (THEMES as readonly string[]).includes(theme) ? (theme as Theme) : 'serenity';
    if (nextTheme === 'serenity') document.documentElement.removeAttribute('data-theme');
    ...
    document.querySelectorAll<HTMLButtonElement>('.theme-picker button').forEach((b) => {
      const on = b.dataset.theme === nextTheme;
      b.classList.toggle('active', on);
      b.setAttribute('aria-pressed', on ? 'true' : 'false');
    });
  ...
  const init = async (): Promise<void> => {
    ...
    document.querySelectorAll<HTMLButtonElement>('.lang-switch button').forEach((btn) => {
      btn.addEventListener('click', () => setLang(btn.dataset.lang || 'fr'));
    });
    document.querySelectorAll<HTMLButtonElement>('.theme-picker button').forEach((btn) => {
      btn.addEventListener('click', () => setTheme(btn.dataset.theme || 'serenity'));
    });

    let savedTheme = 'serenity';
    try {
      const t = localStorage.getItem('theme');
      if (t && (THEMES as readonly string[]).includes(t)) savedTheme = t;
    } catch { /* storage unavailable */ }
    setTheme(savedTheme);

    try {
      [site, reviews] = await Promise.all([
        loadJSON<SiteConfig>('content/site.json'),
        loadJSON<ReviewsData>('content/reviews.json').catch(() => ({ summary: {}, items: [] })),
      ]);
    } catch (err) {
      ...
      document.body.innerHTML = `<div style="padding:40px;font-family:sans-serif;max-width:640px;margin:0 auto">
        ...
        <p>If you opened this file directly, the embedded content bundle is
        missing or stale. Regenerate it by running this once in a terminal,
        from the project folder:</p>
        <pre style="background:#f5f5f5;padding:12px;border-radius:8px">npm run bundle:content</pre>
        <p>Or serve the folder over HTTP: <code>npm run serve</code>
        then open <code>http://localhost:4321/</code>.</p>
