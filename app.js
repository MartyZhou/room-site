(() => {
  ...
  //    window.__CONTENT__), regenerated by scripts/build_bundle.py.
  const isFile = location.protocol === 'file:';
  const fromBundle = (url) => (window.__CONTENT__ || {})[url];

  const loadJSON = async (url) => {
    if (isFile) {
      const b = fromBundle(url);
      if (b !== undefined) return b;
      throw new Error(`Not in bundle: ${url} — run scripts/build_bundle.py`);
    }
    try {
  ...
  const init = async () => {
    ...
    try {
      ...
      document.body.innerHTML = `<div style="padding:40px;font-family:sans-serif;max-width:640px;margin:0 auto">
        ...
        <pre style="background:#f5f5f5;padding:12px;border-radius:8px">python3 scripts/build_bundle.py</pre>
        <p>Or serve the folder over HTTP: <code>python3 -m http.server</code>
